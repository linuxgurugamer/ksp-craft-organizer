...
namespace KspCraftOrganizer
{
    public class OrganizerWindowCraftList
    {
            ...
            using (new GUILayout.VerticalScope(GUILayout.Width(parent.windowWidth - manageTagsWidth - 30 - OrganizerWindow.FILTER_TOOLBAR_WIDTH)))
            {
                ...
                using (LayoutScrollScope shipsScroll = new LayoutScrollScope(shipsRect, shipsScrollPosition))
                {
                    ...
                    using (new GUILayout.VerticalScope(GUILayout.ExpandWidth(false)))
                    {
                        if (model.filteredCrafts.Length == 0)
                        {
                            ...
                            {
                                GUILayout.Label("<Nothing to display - change filter>", GUILayout.Width(400));
                            }
                            else
                            {
                                GUILayout.Label("<You have no ships to load>", GUILayout.Width(400));
                            }
                        }
                        int index = 0;
                ...
                if (parent.showManageTagsToolbar)
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        GUILayout.Label("Filtered: " + model.filteredCrafts.Length + "/" + model.availableCrafts.Count + ", selected: " + model.selectedCraftsCount, GUILayout.Width(200)); //, GUILayout.ExpandWidth(false));
                        GUILayout.Label("  ");
                    }
                }
            }
        ...
        private void drawDirectory(int index, OrganizerCraftEntity dir)
        {
            using (new GUILayout.HorizontalScope())
            {
                if (parent.showManageTagsToolbar)
                ...
                {
                    GUIStyle thisCraftButtonStyle = dir.isSelected ? parent.toggleButtonStyleTrue : parent.toggleButtonStyleFalse;
                    ...
                    using (new GUI.ClipScope(Globals.ZERO_RECT))
                    {
                        ...
                        if (Event.current.type == EventType.Repaint)
                        {
                            dir.guiWidth = GUILayoutUtility.GetLastRect().width;
                        }
                    }
                    ...
                    if (GUILayout.Button("", thisCraftButtonStyle, GUILayout.Height(dir.guiHeight), GUILayout.ExpandHeight(false)))
                    {
                        ...
                        {
                            parent.selectedCraftName = dir.DirName;
                            parent.dirSelected = true;
                            if (dir.isSelected && (Time.realtimeSinceStartup - lastClickTime) < DOUBLE_CLICK_THRESHOLD)
                            {
                                if (dir.DirName == "..")
                                    DirectoryServices.PopDir();
                                else
                                    DirectoryServices.PushDir(dir.DirName);
                                parent.dirSelected = false;
                                parent.Reload();
                            }
                    ...
                    int thumbPosY = (int)thisShipRect.y + (int)(smallThumb ? 10 : (dir.guiHeight - 52) / 2);
                    ...
                    drawLabel(RegisterToolbar.craftNameStyle, firstRowsLeftOffset, nextTop, dir.DirName, "!!dir" + index);
                    nextTop += 20;
                    ...
                    //
                    GUI.Label(new Rect(thumbPosX, thumbPosY, thumbSize, thumbSize), new GUIContent("", "!!dir" + index));
                    GUI.DrawTexture(new Rect(thumbPosX, thumbPosY, thumbSize, thumbSize), dir.DirName == ".." ? RegisterToolbar.folderUpTextureBtn : RegisterToolbar.folderTextureBtn);
        ...
        private void drawSingleCraft(int index, OrganizerCraftEntity craft)
        {
            using (new GUILayout.HorizontalScope())
            {
                ...
                using (new GUILayout.VerticalScope(GUILayout.ExpandWidth(true)))
                {
                    ...
                    int costMaxWidth = thumbPosX - costPosX - 10;
                    int extraInfoMaxWidth = costPosX - firstRowsLeftOffset - 10;
                    drawLabel(RegisterToolbar.craftNameStyle, firstRowsLeftOffset, nextTop, craft.nameToDisplay, "!!craft" + index);
                    nextTop += 20;
                    if (parent.showManageTagsToolbar)
                    {
                    ...
                    foreach (CraftTagGroup tagGroup in tagsGrouper.groups)
                    {
                        drawLabel(RegisterToolbar.tagsStyle, leftOffset, nextTop, tagGroup.displayName + ":");
                        drawMultilineLabel(RegisterToolbar.tagsColor,
                                           tagValuesWidth,
                    ...
                    if (tagsGrouper.restTags.Count > 0)
                    {
                        drawLabel(RegisterToolbar.tagsStyle, leftOffset, nextTop, restOfTagsLabel);
                        drawMultilineLabel(RegisterToolbar.tagsColor,
                                           tagValuesWidth,
                                           (int)(leftOffset + 10 + craft.tagGroupNameWidth),
                                           nextTop,
