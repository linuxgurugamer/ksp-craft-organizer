...
namespace KspNalCommon
{
    ...
    public class DropDownList<T>
    {
        ...
        //GUIStyle dropdownListStyle = null;
        GUIStyle dropdownListStyle { get { return KspCraftOrganizer.RegisterToolbar._dropdownListStyle; } }
        public void onGui(IGuiOverlayContainer overlayContainer, int width)
        {
            ...
            //if (dropdownListStyle == null)
            //	dropdownListStyle = new GUIStyle(GUI.skin.button);
...
#if false
			float dropDownArrowSize = dropdownListStyle.CalcSize(content).y;
			dropdownListStyle.padding.right = (int)dropDownArrowSize;
			dropdownListStyle.alignment = TextAnchor.MiddleLeft;
#endif
            Rect dropDownRectTemp = GUILayoutUtility.GetRect(content, dropdownListStyle, GUILayout.Width(width));
            if (Event.current.type == EventType.Repaint)
            {
                dropDownRect = dropDownRectTemp;
                Vector2 perfectSize = dropdownListStyle.CalcSize(content);
                ...
                if (perfectSize.x > dropDownRect.width)
                {
                    while (perfectSize.x > dropDownRect.width && content.text.Length > 3)
                    {
                        ...
                        perfectSize = dropdownListStyle.CalcSize(content);
                    }
                }
            }

            if (GUI.Button(dropDownRect, content, dropdownListStyle))
            {
...
#if false
					if (itemStyle == null) itemStyle = new GUIStyle();
...
#endif
                    if (currentIndex == selectedItemIndex)
                        itemStyle = KspCraftOrganizer.RegisterToolbar.itemStyleHighlight;
                    else
                        itemStyle = KspCraftOrganizer.RegisterToolbar.itemStyleNormal;
