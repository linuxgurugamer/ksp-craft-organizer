...
namespace KspCraftOrganizer
{
    ...
    public class OrganizerControllerPrimaryTagState
    {
        ...
        public CraftTypeDependendSetting<bool> NOT;
        ...
        public OrganizerControllerPrimaryTagState(OrganizerControllerStateManager parent)
        {
            ...
            NOT = new CraftTypeDependendSetting<bool>(parent, () => parent.markPrimaryProfileDirty("Tag changed selected state"));
        }
    }
    ...
    public class OrganizerControllerPrimaryTagGroupState
    {
        ...
        public CraftTypeDependendSetting<bool> hasRequireAllInFilter;
        ...
        public OrganizerControllerPrimaryTagGroupState(OrganizerControllerStateManager parent)
        {
            ...
            hasRequireAllInFilter = new CraftTypeDependendSetting<bool>(parent, () => parent.markPrimaryProfileDirty("group's 'require all' option changed"));
        }
    }
    ...
    public class OrganizerControllerStateManager
    {
        ...
        public void setNOT(string tagName, bool selected)
        {
            getPrimaryTagState(tagName).NOT.current = selected;
        }

        public bool isNOT(string tagName)
        {
            return getPrimaryTagState(tagName).NOT.current;
        }
        ...
        public bool isGroupHasRequireAllFilter(string groupName)
        {
            return getPrimaryGroupState(groupName).hasRequireAllInFilter.current;
        }
        public void setGroupHasRequireAllFilter(string groupName, bool collapsed)
        {
            getPrimaryGroupState(groupName).hasRequireAllInFilter.current = collapsed;
        }
        ...
        void readFilterSettings(ProfileAllFilterSettingsDto dto, CraftType facility, CraftType craftType)
        {
            ...
            foreach (KeyValuePair<string, OrganizerControllerPrimaryTagState> tagState in primaryTagsState)
            {
                ...
                tagState.Value.NOT.setForFacilityAndCraftType(facility, craftType, false);
            }
            // Set saved tags
            foreach (string selectedTag in filterDto.selectedFilterTags)
            {
                Log.Info("selectedTag: " + selectedTag);
                string[] textSplit = selectedTag.Split(':');
                getPrimaryTagState(textSplit[0]).selectedForFiltering.setForFacilityAndCraftType(facility, craftType, true);
                // the following is where the NOT is set when loading
                if (textSplit.Length > 1)
                    getPrimaryTagState(textSplit[0]).NOT.setForFacilityAndCraftType(facility, craftType, textSplit[1] == "True");
                else
                    Log.Info("Missing NOT 1");
#if false
                getPrimaryTagState(selectedTag).selectedForFiltering.setForFacilityAndCraftType(facility, craftType, true);
                // the following is where the NOT is set when loading
                getPrimaryTagState(selectedTag).NOT.setForFacilityAndCraftType(facility, craftType, false);
#endif
            }
            ...
            foreach (KeyValuePair<string, OrganizerControllerPrimaryTagGroupState> groupState in primaryGroupsState)
            {
                ...
                groupState.Value.hasRequireAllInFilter.setForFacilityAndCraftType(facility, craftType, false);
            }
            // hasRequireAllInFilter
            ...
            foreach (string groupName in filterDto.filterGroupsWithRequireAllOption)
            {
                getPrimaryGroupState(groupName).hasRequireAllInFilter.setForFacilityAndCraftType(facility, craftType, true);
            }
        ...
        void saveFilterSettings(ProfileAllFilterSettingsDto dto, CraftType facility, CraftType craftType)
        {
            ...
            foreach (KeyValuePair<string, OrganizerControllerPrimaryTagState> tagState in primaryTagsState)
            {
                ...
                {
                    // Save the NOT tag here
                    bool b = tagState.Value.NOT.getForFacilityAndCraftType(facility, craftType);
                    selectedFilterTags.Add(tagState.Key + ":" + b.ToString());
                    Log.Info("Saving tag: " + tagState.Key + ":" + b.ToString());
                }
            }
            ...
            List<string> filterGroupsWithRequireAllOption = new List<string>();
            foreach (KeyValuePair<string, OrganizerControllerPrimaryTagGroupState> groupState in primaryGroupsState)
            {
                ...
                if (groupState.Value.hasRequireAllInFilter.getForFacilityAndCraftType(facility, craftType))
                {
                    filterGroupsWithRequireAllOption.Add(groupState.Key);
                }
            ...
            filterDto.filterGroupsWithRequireAllOption = filterGroupsWithRequireAllOption;
